...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const currentStaff = await db.query("SELECT * FROM public.staff WHERE id = $1", [id])

    if (currentStaff.rows.length === 0) {
      return NextResponse.json({ error: "Staff member not found" }, { status: 404 })
    }

    const currentData = currentStaff.rows[0]
    ...
    const result = await db.query(
      `UPDATE staff SET
        ...
        status = $6, image_url = $7, education = $8, skills = $9, experience = $10,
        assigned_section = $11, performance = $12, error_count = $13, birth_date = $14,
        nationality = $15, language = $16, address = $17, updated_at = $18
