...
//         "INSERT INTO public.orders (table_number, status) VALUES ($1, $2) RETURNING id",
...
//   try {
//     const result = await db.query(`
//       SELECT o.*, oi.menu_id, oi.quantity, oi.special_instructions, m.name as item_name, m.price
//       FROM orders o
//       JOIN order_items oi ON o.id = oi.order_id
//       JOIN menu m ON oi.menu_id = m.id
//       ORDER BY o.timestamp DESC
...
//             table_number: row.table_number,
//             status: row.status,
//             payment_status: row.payment_status, // Add payment_status here
//             timestamp: row.timestamp,
//             customer_rating: row.customer_rating,
//             total_price: Number.parseFloat(row.total_price),
//             items: [],
